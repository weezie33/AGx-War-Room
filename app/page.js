...
const DEFAULT_LEGEND = {
  ...
  label: { label: "Ground Label", color: "#e5e7eb" },
...
function createGroundItem(type, q, r, text = "") {
  return {
    ...
    color: DEFAULT_LEGEND[type]?.color || "#ffffff",
    locked: false,
  };
}
...
function defaultBoard(id, name, theme) {
  return {
    id,
    name,
    theme,
...
function createDefaultRoom() {
  const boards = {};
  [
    ["canyon", "Canyon Clash", "canyon"],
    ["defense", "Defense Board", "grass"],
    ["offense", "Offense Board", "desert"],
    ["reserve", "Reserve / Staging", "mixed"],
  ].forEach(([id, name, theme]) => {
    boards[id] = seedBoard(defaultBoard(id, name, theme));
  });
...
function boardToTheme(boardId, theme) {
  if (boardId === "canyon" || theme === "canyon") return "canyon";
  if (theme === "desert") return "desert";
  if (theme === "mixed") return "mixed";
  return "grass";
}

function terrainStyle(theme) {
  if (theme === "canyon") {
    return {
      background:
        "linear-gradient(120deg, rgba(193,157,108,0.94) 0 46%, rgba(255,255,255,0.18) 46.4%, rgba(100,211,255,0.55) 47%, rgba(137,176,108,0.84) 47.6% 76%, rgba(170,120,214,0.40) 76.3%, rgba(193,157,108,0.92) 76.9%)",
    };
  }
  if (theme === "desert") {
    return {
      background:
        "radial-gradient(circle at 20% 20%, rgba(255,255,255,0.06), transparent 18%), linear-gradient(180deg,#bba06f 0%, #a88d5d 44%, #8f754e 100%)",
    };
  }
  if (theme === "mixed") {
    return {
      background:
        "linear-gradient(135deg,#7c7c5c 0%, #8aa36b 42%, #ad9862 42.6%, #c2ae79 100%)",
    };
  }
  return {
    background:
      "radial-gradient(circle at 20% 20%, rgba(255,255,255,0.05), transparent 18%), linear-gradient(180deg,#7f925d 0%, #71844f 44%, #5f7245 100%)",
  };
}
...
export default function Page() {
  ...
  const theme = boardToTheme(room.activeBoardId, board.theme);
        ...
        {occupied.map((cell, idx) => {
          ...
          return (
            <polygon
              ...
              fill={entity.color}
              fillOpacity={idx === 0 ? 0.86 : 0.55}
              stroke={isSelected ? "#ffffff" : "rgba(255,255,255,0.35)"}
              strokeWidth={isSelected ? 2.2 : 1.05}
            />
          );
        })}
        <text x={center.x} y={center.y - 2} textAnchor="middle" fontSize="11" fontWeight="700" fill="#fff">
          {entity.label}
        </text>
        {entity.callsign ? (
          <text x={center.x} y={center.y + 11} textAnchor="middle" fontSize="8" fill="rgba(255,255,255,0.88)">
            {entity.callsign}
          </text>
        ) : null}
        {entity.locked ? (
          <text x={center.x + 18} y={center.y - 18} textAnchor="middle" fontSize="12" fill="#fff">
            🔒
          </text>
        ) : null}
      </g>
    ...
    return (
      ...
      >
        <rect
          ...
          height={26}
          rx={10}
          fill={item.kind === "note" ? "rgba(254,240,138,0.92)" : "rgba(255,255,255,0.82)"}
          stroke={selected ? "#111827" : "rgba(17,24,39,0.3)"}
  ...
  function renderArrow(arrow) {
    ...
    return (
      <g key={arrow.id}>
        ...
        />
        <text x={(s.x + e.x) / 2} y={(s.y + e.y) / 2 - 8} textAnchor="middle" fontSize="11" fontWeight="700" fill="#fff">
          {arrow.label}
        </text>
        {selected ? (
          <>
      ...
      <div className="warroom-grid">
        <aside className="sidebar-panel">
          ...
          {selectedTab === "objects" && (
            <div className="section stack-gap">
              ...
              <div className="subtle">Click the grid to place the selected object. Drag items after placing. HQ occupies seven hexes.</div>
            </div>
          )}
          ...
          {selectedTab === "voice" && (
            <div className="section stack-gap">
              ...
              <div className="subtle">This includes a voice command layout and quick links. Built-in browser voice is not included; use Discord, Meet, or Slack links here.</div>
        ...
        <section className="main-panel">
          ...
          <div className="warroom-layout">
            ...
            <div className="battlefield-shell" style={terrainStyle(theme)}>
              <svg
                ref={svgRef}
              ...
              >
                {theme === "canyon" && (
                  <>
                    <polyline points="-40,230 120,330 220,380 310,430 390,520 480,580 580,680 740,760" fill="none" stroke="rgba(255,255,255,0.75)" strokeWidth="10" strokeLinejoin="round" />
                    <polyline points="-40,230 120,330 220,380 310,430 390,520 480,580 580,680 740,760" fill="none" stroke="rgba(98,227,255,0.55)" strokeWidth="18" strokeLinejoin="round" />
                    <polyline points="-20,940 130,860 210,760 280,700 350,620 430,560 520,480 610,420" fill="none" stroke="rgba(177,103,255,0.32)" strokeWidth="24" strokeLinejoin="round" />
                  </>
                )}
                ...
                {CELLS.map((cell) => {
                  ...
                  return (
                    <g key={`${cell.q}-${cell.r}`}>
                      <polygon points={hexPoints(cell.x, cell.y)} fill={occupied ? `${occupied.color}20` : "rgba(255,255,255,0.02)"} stroke={`rgba(255,255,255,${gridOpacity / 100})`} strokeWidth="1.02" />
                      {showCoords && <text x={cell.x} y={cell.y + 3} textAnchor="middle" fontSize="7.5" fill="rgba(255,255,255,0.58)">{cell.q},{cell.r}</text>}
                    </g>
                  );
                })}
                  ...
                  .map((cursor) => (
                    <g key={cursor.id} pointerEvents="none">
                      ...
                      <rect
                        x={cursor.x + 12}
                        y={cursor.y - 13}
                        width={Math.max(68, (cursor.name || "Operator").length * 7)}
                        height="24"
                        rx="12"
                        fill="rgba(17,24,39,0.88)"
                        stroke={cursor.color || "#60a5fa"}
                      />
                      <text x={cursor.x + 20} y={cursor.y + 2} fontSize="11" fontWeight="700" fill="#fff">
                        {cursor.name}
                      </text>
                    </g>
                  ))}
