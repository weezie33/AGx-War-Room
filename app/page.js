...
const MUD_ROW = [
  { position: 1, X: 518, Y: 535 }, { position: 2, X: 519, Y: 534 },
  { position: 3, X: 519, Y: 533 }, { position: 4, X: 520, Y: 532 },
  { position: 5, X: 520, Y: 531 }, { position: 6, X: 521, Y: 530 },
  { position: 7, X: 521, Y: 529 }, { position: 8, X: 522, Y: 528 },
  { position: 9, X: 522, Y: 527 }, { position: 10, X: 523, Y: 526 },
  { position: 11, X: 523, Y: 525 }, { position: 12, X: 524, Y: 524 },
  { position: 13, X: 524, Y: 523 }, { position: 14, X: 525, Y: 522 },
  { position: 15, X: 525, Y: 521 }, { position: 16, X: 526, Y: 520 },
  { position: 17, X: 526, Y: 519 }, { position: 18, X: 527, Y: 518 },
  { position: 19, X: 527, Y: 517 }, { position: 20, X: 528, Y: 516 },
  { position: 21, X: 528, Y: 515 }, { position: 22, X: 529, Y: 514 },
  { position: 23, X: 529, Y: 513 }, { position: 24, X: 530, Y: 512 },
  { position: 25, X: 530, Y: 511 }, { position: 26, X: 531, Y: 510 },
  { position: 27, X: 531, Y: 509 }, { position: 28, X: 532, Y: 508 },
  { position: 29, X: 532, Y: 507 }, { position: 30, X: 533, Y: 506 },
  { position: 31, X: 533, Y: 505 }, { position: 32, X: 534, Y: 504 },
  { position: 33, X: 534, Y: 503 }, { position: 34, X: 535, Y: 502 },
...
function seedBoard(board) {
  if (board.id === "canyon") {
    ...
    p2.callsign = "Hexie";
    board.entities.push(
      hq1, hq2, createEntity("refinery", 16, 6), createEntity("medical", 25, 12),
      createEntity("military", 28, 8), createEntity("mine", 7, 21),
      createEntity("boss", 31, 4), p1, p2
    );
...
export default function Page() {
  ...
  function svgPointToHex(clientX, clientY) {
    ...
    return { q, r, x, y, ...serverCoordFor(q, r) };
  }
  ...
  function deleteObjectAtHex(point) {
    ...
    patchBoard((draft) => {
      const entityIndex = draft.entities.findIndex((entity) => {
        ...
        if (entity.size === 7) {
          occupied.push([entity.q + 1, entity.r],[entity.q - 1, entity.r],[entity.q, entity.r + 1],[entity.q, entity.r - 1],[entity.q + 1, entity.r - 1],[entity.q - 1, entity.r + 1]);
        }
      ...
      const arrowIndex = draft.arrows.findIndex((arrow) => (arrow.startQ === point.q && arrow.startR === point.r) || (arrow.endQ === point.q && arrow.endR === point.r));
  ...
  function goToCoordinate() {
    ...
    if (best) {
      setHoverCoord({ X: best.serverX, Y: best.serverY, q: best.q, r: best.r });
  ...
  function renderEntityShape(entity) {
    ...
    if (entity.size === 7) {
      occupied.push(
        { q: entity.q + 1, r: entity.r }, { q: entity.q - 1, r: entity.r },
        { q: entity.q, r: entity.r + 1 }, { q: entity.q, r: entity.r - 1 },
        { q: entity.q + 1, r: entity.r - 1 }, { q: entity.q - 1, r: entity.r + 1 }
      );
    }
    ...
    return (
      ...
      >
        ...
        <text x={center.x} y={center.y - 2} textAnchor="middle" fontSize="10" fontWeight="700" fill="#ffffff">{entity.label}</text>
        {entity.callsign ? <text x={center.x} y={center.y + 10} textAnchor="middle" fontSize="7.5" fill="rgba(255,255,255,0.95)">{entity.callsign}</text> : null}
      </g>
    );
  }
  ...
  function renderGroundItem(item) {
    ...
    return (
      ...
      >
        <rect x={p.x - width / 2} y={p.y - 18} width={width} height={26} rx={10}
          fill={item.kind === "note" ? "rgba(254,240,138,0.96)" : "rgba(255,255,255,0.96)"}
          stroke={selected ? "#111827" : "rgba(17,24,39,0.28)"} strokeWidth={selected ? 2 : 1}/>
        <text x={p.x} y={p.y - 1} textAnchor="middle" fontSize="11" fontWeight="700" fill="#111827">{item.text}</text>
      </g>
    );
  }
  ...
  function renderArrow(arrow) {
    ...
    return (
      <g key={arrow.id} onPointerDown={(evt) => {
        evt.stopPropagation();
        setSelectedItem({ kind: "arrow", id: arrow.id });
        ...
        <text x={(s.x + e.x) / 2} y={(s.y + e.y) / 2 - 8} textAnchor="middle" fontSize="11" fontWeight="700" fill="#111827">{arrow.label}</text>
        {selected && selectedTool === "select" ? (
          <>
            <circle cx={s.x} cy={s.y} r="8" fill="#ffffff" stroke={arrow.color} strokeWidth="3" onPointerDown={(e2) => { e2.stopPropagation(); if (canMove(arrow)) setDragState({ kind: "arrow-start", id: arrow.id }); }}/>
            <circle cx={e.x} cy={e.y} r="8" fill="#ffffff" stroke={arrow.color} strokeWidth="3" onPointerDown={(e2) => { e2.stopPropagation(); if (canMove(arrow)) setDragState({ kind: "arrow-end", id: arrow.id }); }}/>
          </>
        ) : null}
      </g>
    );
        ...
        <aside className="sidebar-panel">
          <div className="header-block">
            <div>
              ...
              <p className="subtle">Fully accurate v6. Light grey background, square-fill hex board, no blue/purple lines, add/delete/modify objects.</p>
            </div>
          ...
          <div className="section">
            ...
            <div className="instruction-box">
              <div><strong>1.</strong> Choose a board and set your role.</div>
              <div><strong>2.</strong> Pick a tool: Player, HQ, Med Tent, Mine, Label, Arrow, or Delete.</div>
              <div><strong>3.</strong> Click the map to add new objects.</div>
              <div><strong>4.</strong> Use Select to click and drag objects.</div>
              <div><strong>5.</strong> Use Delete tool to remove objects from the map instantly.</div>
              <div><strong>6.</strong> Use Inspect to rename, recolor, lock, unlock, or delete selected objects.</div>
              <div><strong>7.</strong> Use Formations to save and load layouts.</div>
              <div><strong>8.</strong> Hover tiles to see real server X:Y coordinates.</div>
            </div>
          </div>
          ...
          <div className="section">
            ...
            <div className="subtle">{hoverCoord ? `Current: X:${hoverCoord.X} Y:${hoverCoord.Y}` : "Hover over the board to inspect coordinates."}</div>
          </div>

          <div className="tab-row">
            {["boards", "objects", "formations", "timeline", "voice", "inspect"].map((tab) => (
              <button key={tab} className={selectedTab === tab ? "mini-tab active" : "mini-tab"} onClick={() => setSelectedTab(tab)}>{tab}</button>
            ))}
          </div>
              ...
              <div className="legend-grid">
                {Object.entries(board.legend).map(([key, value]) => (
                  <div key={key} className="legend-card">
                    ...
                    <input type="color" value={value.color} onChange={(e) => patchBoard((draft) => {
          ...
          {selectedTab === "objects" && (
            <div className="section stack-gap">
              ...
              <div className="subtle">Add new objects by choosing a tool and clicking the map. Delete objects with the Delete tool.</div>
            </div>
          )}
